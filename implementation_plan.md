...
*[🇯🇵 日本語版 (Japanese Version) はこちら](./implementation_plan_ja.md)*
...
### 6.3 多轮会话上下文记忆强化 (Conversational Memory & Query Rewriting)
目前系统为单轮（Single-turn）问答，为使 AI 能够准确理解存在指代（如“他”、“第二天”）的追问，计划重构 `/api/chat` 的 RAG 检索链路：
1. **记忆挂载**：引入全局的 `SessionID`，利用预留的 Amazon DynamoDB 保存前 N 轮对话历史。
2. **检索重写 (Query Rewriting)**：在核心提问到达 Pinecone 向量检索之前，先经过第一轮极速 LLM 预处理。利用历史上下文将“残缺的追问”补全为“独立完整的提问”（消除代词），以保证命中准确的知识块。
3. **最终 Prompt 合成**：在提交问答核心链时，将补全后的独立查询词、命中的向量切片，以及最近的 `Chat History` 一并打包发给主力推理模型。

## 7. 技术坑位与解决方案汇总 (Serverless RAG 实战经验)

在项目初期开发中，我们遭遇并克服了以下针对 AWS Lambda 架构的硬核技术挑战：

### 7.1 AWS Lambda 运行环境限制
*   **SHM 共享内存缺失导致的崩溃**：
    *   **现象**：使用 `langchain-pinecone` 默认的 `add_documents` 时，后台由于调用 `multiprocessing.ThreadPool` 导致 `SemLock` 创建失败报错。
    *   **解决方案**：绕过封装库，手写**全同步（Serial）**的 Pinecone HTTP Upsert 逻辑，彻底降维打击并发冲突。
*   **只读文件系统冲突**：
    *   **现象**：`tiktoken` 默认试图在 Lambda 的只读 `$HOME` 目录下创建缓存。
    *   **解决方案**：通过环境变量强制重定向 `TIKTOKEN_CACHE_DIR` 至 `/tmp` 这一唯一可写区域。

### 7.2 API Gateway 29秒超时瓶颈
*   **现象**：调用参数过大（如 122B 模型）的大模型时，推理时长极易撞上 API Gateway 的 29 秒硬上限。
*   **解决方案**：目前采用 8B 级高敏捷模型应对；长线规划切换至 **Lambda Function URL** (15分钟上限) 或 **SQS 异步模式**。

### 7.3 RAG 检索召回精准度调优
*   **维度与模型不匹配**：
    *   **重坑**：误用生成式 LLM 做向量化（维度高但语义稀疏），导致库里有原文却搜不出来。
    *   **解决方案**：切换至**专用对比学习模型** `Qwen3-Embedding-0.6B`，并锁定 **1024 维** 黄金孔径。
