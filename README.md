...
- **⚡️ 非同期 RAG パイプライン (Async Polling)**：API Gateway の 29s 制限を完全に突破。内部で S3 イベントと DynamoDB を活用したタスク管理を実現。
- **🧠 デュアル検索モード (Dual-Mode RAG)**：「節約自作版 (Lambda+ModelScope)」と「フルマネージド版 (AWS Bedrock KB)」をワンクリックで切替可能。
- **🚀 自動インデックス (Auto-Ingestion)**：S3 アップロード後に Bedrock ナレッジベースの同期を自動トリガー。
